...
class Comment_model extends CI_Model
{
    ...
    function tree_all($id) {
        $result = $this->db->query("SELECT * FROM comment where id_ar = $id")->result_array();
    ...
    function tree_by_parent($id,$in_parent) {
        $result = $this->db->query("SELECT * FROM comment where parent_id = $in_parent AND  id_ar = $id")->result_array();
        foreach ($result as $row) {
            $data[] = $row;
        }
        return $data;
    ...
    {
        $this->db->set("id_ar", $this->input->post('id_ar'));
